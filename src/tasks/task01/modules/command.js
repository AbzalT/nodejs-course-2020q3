...
function setOptions(){
    Object.values(OPTIONS_REQUIRED).forEach(option => program.requiredOption(option.flags, option.description))
    Object.values(OPTIONS).forEach(option => program.option(option.flags, option.description))
}
...
function parseConsole(){
    ...
    if (process.argv.length < 3) {
        ...
        [...Object.entries(program._optionValues)].forEach(option => args[option[0]] = option[1]);
...
function validateArgs() {
    if(!ACTION.includes(args.action)) {
        errorHandler(new Error('the option "action" value must be "encode" or "decode"'));
    }
